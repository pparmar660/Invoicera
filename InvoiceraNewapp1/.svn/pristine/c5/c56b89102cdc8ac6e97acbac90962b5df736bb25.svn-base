...
public class CanvasView extends View {
	...
	public CanvasView(Context context, ArrayList<GraphItemModel> itemList) {
		...
		for (int i = 0; i < itemList.size(); i++) {
			...
			if (model.get(i).getNumberOfIem()>0)
				nonzeroItem++;
		}
		...
		for (int i = 0; i < itemList.size(); i++) {
			float angle = ((model.get(i).getNumberOfIem() * (360- (GraphPagerAdapter.gapAngle * nonzeroItem))) / numberOfItem);
...
/*
			if (model.get(i).getNumberOfIem()==0)
				AngleList.add(GraphPagerAdapter.gapAngle);
			else AngleList.add(angle);*/
	...
	protected void onDraw(Canvas canvas) {
		...
		super.onDraw(canvas);
		Log.e("item list",AngleList.toString());
		...
		canvas.drawText(numberOfItem + "", GraphPagerAdapter.centerX, GraphPagerAdapter.centerY,
			...
			canvas.drawText("CREATE " + "", GraphPagerAdapter.centerX, GraphPagerAdapter.centerY,
					paintOfText);
	...
	void drawHint(Canvas canvas) {
		...
		for (int i = 0; i < hintArrayPos.length; i++) {
			hintArrayPos[i][0] = HomePageFragment.global.screenWidth * 79 / 100;
