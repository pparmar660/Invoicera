 ...
 */
public class Invoice_Filter extends BaseActivity implements View.OnClickListener, WebApiResult {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.invoice_filter);

        Intent intent = getIntent();

        if (intent != null)
            if (intent.getStringExtra(Constant.KEY_INVOICE_STATUS) != null) {
                //   if (!intent.getStringExtra(Constant.KEY_INVOICE_STATUS).equalsIgnoreCase("Recent"))
                invoice_status = intent.getStringExtra(Constant.KEY_INVOICE_STATUS);
        ...
        currentInvoiceStTv = (TextView) findViewById(R.id.invoice_st_tv);
        invoiceStLiner = (LinearLayout) findViewById(R.id.invoice_st_liner);
        ...
        frames.add((FrameLayout) findViewById(R.id.invoiceFrame));
        ...
        setSelectedPos(R.id.invoice_st);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.clear:
                ...
                selectedid = invoiceStatusRadioGroup.getCheckedRadioButtonId();
                if (selectedid > 0) {
                    invoiceStatusRadioGroup.clearCheck();
                }
            ...
            case R.id.apply:
                ...
                if (idx >=0)
                    invoice_status = InvoiceStatusArray[idx];
                ...
                returnIntent.putExtra(Constant.KEY_INVOICE_STATUS, invoice_status);
                returnIntent.putExtra(Constant.KEY_DATE_FROM, fromDate);
                returnIntent.putExtra(Constant.KEY_DATE_TO, todate);
                setResult(Constant.requestCodeInvoiceFilter, returnIntent);
                finish();
                break;
    ...
    public void setSelectedPos(int id) {
        ...
        for (int i = 0; i < textViews.size(); i++) {
            ...
            if (textViews.get(i).getId() == id) {
                ...
                if (!invoice_status.equalsIgnoreCase("Recent")) {
                    invoiceStatusRadioGroup.setVisibility(View.GONE);
                    currentInvoiceStTv.setVisibility(View.VISIBLE);
                    currentInvoiceStTv.setText(invoice_status);
                } else {

                    invoiceStatusRadioGroup.setVisibility(View.VISIBLE);
                    currentInvoiceStTv.setVisibility(View.GONE);
                ...
                try {
                    ...
                    for (int j = 0; j < object.getJSONObject("clients").getJSONArray("client").length(); j++) {
                        ...
                        if (selectedClientId > 0) {
                            ...
                            if (selectedClientId == Integer.parseInt(object.getJSONObject("clients").getJSONArray("client")
                                    .getJSONObject(j).getString("client_id")))
